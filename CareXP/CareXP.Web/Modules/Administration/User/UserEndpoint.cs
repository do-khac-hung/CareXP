...
namespace CareXP.Administration.Endpoints
{
    using Entities;
    using Repositories;
    using Serenity;
    ...
    public class UserController : ServiceEndpoint
    {
        ...
        /// <summary>
        /// Đây là hàm nội dung của dynamic script với key='UserData' sẽ phục vụ cấp dữ liệu cho client side
        /// về tài khoản đăng nhập hiện thời
        /// Chung ta sẽ không cache nó bằng dynamic script manager, bởi vì dynamic scripts sẽ được cached toàn cục,
        /// tương tự như biến tĩnh, không phụ thuộc vào từng user.
        /// </summary>
        ...
        public ScriptUserDefinition GetUserData()
        {
            ...
            result.TenantId = user.TenantId; // Khi client hỏi dữ liệu người dùng hiện thời, server trả về thêm thông tin mới
