...
namespace CareXP.Administration.Repositories
{
    ...
    public class UserPermissionRepository
    {
        ...
        /// <summary>
        ///   Trả về danh sách các quyền sẽ hiển thị lên 2 giao diện quản trị bổ sung cho một Role, hoặc cho một user nào đó
        /// </summary>
        /// <returns></returns>
        public ListResponse<string> ListPermissionKeys()
        {
            return LocalCache.Get("Administration:PermissionKeys", TimeSpan.Zero, () =>
            {
                ...
                //Nếu không có quyền xem log thì cũng không được hiển thị chức năng log.
                //Với tài khoản admin thì yên tâm là vẫn mở được
                if (!Authorization.HasPermission(PermissionKeys.Logs))
                {
                    result.Remove(PermissionKeys.Logs);
                };

                //Nếu không có quyền Dev thì cũng không được hiển thị chức năng demo cho lập trình
                //Với tài khoản admin thì yên tâm là vẫn mở được
                if (!Authorization.HasPermission(PermissionKeys.Devs))
                {
                    result.Remove(PermissionKeys.Devs);
                };

                //Nếu không có quyền Admin thì cũng không được các chức năng điều khiển khác
                if (!Authorization.HasPermission(PermissionKeys.Security))
                {
                    result.Remove(PermissionKeys.Security);
                };

                //Nếu không có quyền Dịch thì cũng không được các chức năng dịch ngôn ngữ
                if (!Authorization.HasPermission(PermissionKeys.Translation))
                {
                    result.Remove(PermissionKeys.Translation);
                };

                //Nếu không có quyền kiểm soát Tenant thì cũng không được xem luôn
                if (!Authorization.HasPermission(PermissionKeys.Tenant.View))
                {
                    result.Remove("Tenant");
                }

                //Nếu không có quyền dịch thì cũng không được xem luôn
                if (!Authorization.HasPermission(PermissionKeys.Translation))
                {
                    result.Remove(PermissionKeys.Translation);
                }
